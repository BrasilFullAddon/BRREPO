...
addon = xbmcaddon.Addon(id='plugin.video.brasilfull')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.BrasilFull Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(BrasilFull,Erro de codigo - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(BrasilFull,Falha de ao se conectar com o servidor. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        if media_info:
            ...
            keyboard = xbmc.Keyboard(nameStr,'Deseja renomear?')
            keyboard.doModal()
            ...
            if xbmcvfs.exists(url):
                if url.startswith("smb://") or url.startswith("nfs://"):
                    ...
                    if copy:
                        ...
                        addon_log("falha ao copiar do smb:")
                else:
        ...
        if len(soup('channels')) > 0:
            ...
            for channel in channels:
                ...
                try:
                    ...
                    addon_log('Problema ao adicionar fonte(): '+name.encode('utf-8', 'ignore'))
        else:
...
def getChannelItems(name,url,fanart):
        ...
        for channel in channel_list('subchannel'):
            ...
            try:
                ...
                addon_log('Problema ao adicionar diretório - '+name.encode('utf-8', 'ignore'))
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('[COLOR green][B]Brasil [/B][/COLOR][COLOR yellow][B]Full[/B][/COLOR] Selecione uma opção:', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
             try:
...
#                    print 'item link', item('link')
                    ...
                    for i in item('gdrive'):
                        if not i.string == None:
                            gdrive = 'plugin://plugin.video.gdrive?mode=streamURL&amp;url=https://docs.google.com/file/d/'+i.string
                            url.append(gdrive)

                elif len(item('drive')) >0:
                    for i in item('drive'):
                        if not i.string == None:
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR green][B]Brasil [/B][/COLOR][COLOR yellow][B]Full[/B][/COLOR] - Selecione uma Opcao:', names)
            if index >= 0:
                playname=names[index]
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('BrasilFull','Escolha um local para salvar os arquivos.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('BrasilFull', 'Você deseja adicionar esse arquivo como fonte?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(BrasilFull,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if playlist:
            if addon.getSetting('add_playlist') == "false":
                ...
                name = name + '[COLOR steelblue] (' + str(len(playlist)) + ' Opcoes )[/COLOR]'
                play_list = True
        else:
            u += "url="+urllib.quote_plus(url)+"&mode="+mode
        if regexs:
